...
类型门禁使用锁定的 mypy，首批范围是 `src/storage/`、`src/config.py`、`src/database.py`、
`src/time_utils.py` 和 `src/v2/time_utils.py`（9 个源文件）。启用未注解函数体检查、隐式可空值限制、
...
错误引擎调用和未收窄的迁移版本空值，并保留每阶段失败即停及锁定工具调用测试。

**尚未达成的检查要求（#20）：** 认证、管理、令牌和其余同步业务模块，以及测试/脚本/Alembic 的
