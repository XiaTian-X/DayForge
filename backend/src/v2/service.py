...
async def _mutate_observation(
    ...
    try:
        ...
        raise DomainError("INVALID_PAYLOAD", str(exc)) from exc
    metric = await _get_metric(session, user_id, str(payload.metric_uuid), include_deleted=False)
    if metric is None:
...
async def _dispatch_operation(
    ...
    handlers = {
        ...
        "metric": _mutate_metric,
        "metric_observation": _mutate_observation,
