...
def test_canonical_json_keeps_unicode_numeric_decimals_and_stable_key_order():
    value = {"z": [Decimal("1"), Decimal("1.25")], "名称": "喝水", "a": {"y": None, "x": True}}
    expected = '{"a":{"x":true,"y":null},"z":[1,1.25],"名称":"喝水"}'
    assert canonical_json(value) == expected
    assert canonical_json(dict(reversed(list(value.items())))) == expected
    assert parse_json(expected) == {"a": {"x": True, "y": None}, "z": [1, 1.25], "名称": "喝水"}
...
def test_nested_utc_encoding_preserves_instant_microseconds_and_json_leaf_types():
    ...
    value = {
        "naive_utc": instant,
        "nested": ({"offset": instant.replace(tzinfo=timezone(timedelta(hours=8)))}, None),
        "date": date(2026, 9, 14),
...
def test_operation_identity_hashes_the_original_wire_request_not_a_normalized_edit():
    ...
    assert operation_hash(operation) == hashlib.sha256(expected_wire.encode("utf-8")).hexdigest()
    reordered = operation.model_copy(update={"payload": dict(reversed(list(operation.payload.items())))})
    ...
    for update in (
        ...
        assert operation_hash(operation.model_copy(update=update)) != operation_hash(operation)
