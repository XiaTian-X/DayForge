...
@pytest.mark.parametrize("fail_stage", ["sync", "ruff", "pytest", "openapi", "none"])
def test_verify_backend_fails_fast_and_never_updates_the_lock(tmp_path, fail_stage):
    ...
    launcher.write_text(
        ...
        "stage = ('sync' if args[0] == 'sync' else 'ruff' if 'ruff' in args else "
    ...
    count = {"sync": 1, "ruff": 2, "pytest": 3, "openapi": 4, "none": 4}[fail_stage]
    assert command_log.exists(), result.stdout + result.stderr
    assert [json.loads(line) for line in command_log.read_text().splitlines()] == expected[:count]
    assert result.returncode == (0 if fail_stage == "none" else 1), result.stdout + result.stderr
