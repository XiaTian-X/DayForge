    ...
    try:
        ...
        async with AsyncClient(transport=ASGITransport(app=app, raise_app_exceptions=False), base_url="http://test") as client:
            ...
            device = await client.post("/api/v2/devices/register", headers=bearer, json={
                "installation_id": "commit-primary-device", "platform": "android", "protocol_version": 4,
            })
            assert device.status_code == 200
            yield client, engine, bearer, {"Authorization": "Token " + api_key}, device.json()["device_id"]
    finally:
        set_engine(previous)
...
async def database_state(engine):
    async with engine.connect() as connection:
        names = (await connection.execute(text("SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%' AND name != 'commit_fault' ORDER BY name"))).scalars().all()
        return {name: (await connection.execute(text(f'SELECT * FROM "{name}" ORDER BY rowid'))).all() for name in names}


@pytest.mark.parametrize("kind", ["push", "timer", "register"])
@pytest.mark.parametrize("authentication", ["jwt", "api-token"])
async def test_commit_failure_never_acknowledges_and_same_request_can_retry(runtime_http, kind, authentication):
    ...
    if kind == "push":
        ...
        initial = await client.post("/api/v2/sync/push", headers=bearer, json=with_device(fixture("client/push-all-entities.json"), device))
        ...
        body = {"installation_id": "commit-secondary-device", "platform": "android", "protocol_version": 4}
    ...
    async with engine.begin() as connection:
        ...
        await connection.execute(text("CREATE TABLE commit_fault (bad_user INTEGER REFERENCES users(id) DEFERRABLE INITIALLY DEFERRED)"))
        await connection.execute(text(f"CREATE TRIGGER inject_commit_failure AFTER INSERT ON {table} BEGIN INSERT INTO commit_fault VALUES (-999999); END"))
    ...
    if kind == "register":
        ...
        replay = await client.post(path, headers=headers, json=body)
        assert replay.json()["results"] == [{**r, "status": "already_applied"} for r in recovered.json()["results"]]


async def test_authentication_and_handler_share_one_committed_session_before_response(runtime_http):
    ...
    async def observed_app(scope, receive, send):
        async def observed_send(message):
            if message["type"] == "http.response.start":
                ...
                async with engine.connect() as connection:
                    assert (await connection.execute(text("SELECT COUNT(*) FROM plan_nodes WHERE public_id = :id"), {"id": operation["entity_uuid"]})).scalar_one() == 1
                    assert (await connection.execute(text("SELECT last_used_at FROM api_tokens"))).scalar_one() is not None
            await send(message)
        await app(scope, receive, observed_send)

    try:
        async with AsyncClient(transport=ASGITransport(app=observed_app), base_url="http://test") as observer:
            response = await observer.post("/api/v2/sync/push", headers=headers, json={"device_id": device, "operations": [operation]})
...
async def test_http_errors_roll_back_api_token_activity_without_committing(runtime_http, failure):
    client, engine, _, headers, device = runtime_http
    from uuid import uuid4

    body = {"device_id": device, "operations": []} if failure == "request-validation" else {
        "device_id": str(uuid4()), "operations": [goal_operation()],
    }
    before = await database_state(engine)
...
def test_all_runtime_session_dependencies_use_the_same_pre_response_scope():
    ...
    from src.main import admin_router, admin_tokens_router, auth_router, tokens_router, v2_router
    ...
    for router in (app, admin_router, admin_tokens_router, auth_router, tokens_router, v2_router):
