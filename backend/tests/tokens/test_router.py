...
class TestCreateUserToken:
    ...
    async def test_expiring_token_authenticates_after_sqlite_round_trip(
        ...
        assert [token["name"] for token in authenticated_resp.json()] == [
            "e2e-expiring"
        ]
    ...
    async def test_create_token_stores_hash_not_raw(
        self, test_client, user_token, async_session
    ):
        resp = await test_client.post(
            "/api/v1/auth/tokens",
            json={"name": "hashed"},
...
class TestListUserTokens:
    ...
    async def test_list_only_shows_own_tokens(
...
class TestAdminCreateToken:
    ...
    async def test_admin_creates_token_for_user(
        self, test_client, admin_token, regular_user
    ):
        resp = await test_client.post(
    ...
    async def test_admin_create_token_with_expiry(
        self, test_client, admin_token, regular_user
    ):
        resp = await test_client.post(
            f"/api/v1/admin/users/{regular_user.id}/tokens",
...
class TestAdminListTokens:
    ...
    async def test_admin_lists_user_tokens(
        self, test_client, admin_token, user_token, regular_user
    ):
    ...
    async def test_admin_list_does_not_expose_raw_token(
        self, test_client, admin_token, user_token, regular_user
    ):
        await test_client.post(
            "/api/v1/auth/tokens",
...
class TestAdminDeleteToken:
    ...
    async def test_admin_deletes_user_token(
        self, test_client, admin_token, user_token, regular_user
    ):
        create_resp = await test_client.post(
            "/api/v1/auth/tokens",
