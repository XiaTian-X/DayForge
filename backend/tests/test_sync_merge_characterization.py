...
async def merge_client(test_client, async_session):
    tokens = await register_account(test_client, async_session, "merge-characterization")
...
async def assert_history(session, operation, revisions):
    ...
    changes = (await session.execute(
        select(SyncChange).where(SyncChange.entity_uuid == entity_uuid)
    ...
    assert [row.payload_json for row in snapshots] == [row.payload_json for row in changes]


@pytest.mark.parametrize("explicit_clear", [False, True], ids=["omitted", "explicit-null"])
async def test_stale_nested_optional_field_distinguishes_omission_from_clear(
    merge_client, async_session, explicit_clear,
):
    base = goal_operation()
    ...
    assert result["entity"]["goal"]["due_date"] == (None if explicit_clear else "2026-12-31")
...
async def test_explicit_default_and_nested_changes_survive_disjoint_remote_edit(merge_client):
    base = goal_operation()
...
async def test_merged_replay_keeps_original_request_identity_and_does_not_write_again(
    ...
    record = (await async_session.execute(select(SyncOperation).where(
...
async def test_multi_field_conflict_is_sorted_cached_and_does_not_write_history(
    ...
    assert result["conflicting_fields"] == ["description", "goal.target_cycles", "title"]
...
async def test_another_accounts_same_uuid_snapshot_cannot_replace_missing_merge_base(
    test_client, async_session, merge_client,
):
    base = goal_operation()
    assert (await merge_client(base))["revision"] == 1
    remote = edit(base)
    ...
    assert (await merge_client(remote))["revision"] == 2
    owner = (await async_session.execute(select(User).where(
        User.username == "merge-characterization",
    ))).scalar_one()
    owner_id = owner.id

    tokens = await register_account(test_client, async_session, "other-merge-owner")
    device = await register_device(test_client, tokens["access_token"], "other-merge-owner-phone")
