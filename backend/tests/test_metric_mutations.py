...
async def metric_context(test_client, async_session):
    account = await register_account(
        test_client, async_session, "metric_mutations_owner"
    )
...
async def test_metric_rejections_are_cached_without_changing_entity_history(
    async_session, metric_context, kind, case
):
    ...
    for model in (EntityRevisionSnapshot, SyncChange):
        ...
        assert {row.entity_uuid: row.revision for row in rows} == {
            metric["entity_uuid"]: 1,
            observation["entity_uuid"]: 1,
        }


@pytest.mark.parametrize("kind", ["metric", "observation"])
async def test_repeated_delete_does_not_append_history_or_restore_tombstone(
    async_session, metric_context, kind
):
    ...
    assert denied["error_code"] == (
        "ENTITY_DELETED" if kind == "metric" else "ENTITY_ALREADY_EXISTS"
    )
        ...
        rows = (
            (
                await async_session.execute(
                    select(model).where(model.entity_uuid == original["entity_uuid"])
                )
            )
            .scalars()
            .all()
...
@pytest.mark.parametrize(
    "reference", ["missing", "deleted", "other-owner", "other-device"]
)
async def test_observation_rejects_unavailable_metric_or_device_impersonation(
    test_client, async_session, metric_context, reference
):
    ...
    if reference == "missing":
        ...
        other_device = await register_device(
            test_client, other["access_token"], "other-metric-phone"
        )
        foreign = metric_operation(name="Private metric")
        created = await push(
            test_client, other["access_token"], other_device, [foreign]
        )
    ...
    assert result["error_code"] == (
        "SOURCE_DEVICE_MISMATCH" if reference == "other-device" else "METRIC_NOT_FOUND"
    )
...
async def test_external_observation_dedup_preserves_attribution_and_next_item(
    async_session, metric_context
):
    submit, metric, _, _, _, device = metric_context
    original = observation_operation(metric["entity_uuid"])
    original["payload"].update(
        source_type="api", external_event_id="scale-reading-1", source_device_id=device
    )
    ...
    assert await submit(duplicate, next_item) == [
        rejected,
    ...
    assert {row.public_id for row in rows} == {
        ...
        next_item["entity_uuid"],
        metric_context[2]["entity_uuid"],
    }
...
@pytest.mark.parametrize(
    "mutation",
    ["metric-update", "metric-delete", "observation-create", "observation-delete"],
)
async def test_metric_journal_failure_rolls_back_mutation_and_allows_next_operation(
    async_session, metric_context, table, mutation
):
    submit, metric, observation, originals, _, _ = metric_context
    ...
    for model in (EntityRevisionSnapshot, SyncChange):
        ...
        assert {row.entity_uuid: row.revision for row in rows} == {
            metric["entity_uuid"]: 1,
            observation["entity_uuid"]: 1,
            following["entity_uuid"]: 1,
        }
    facts = (await async_session.execute(select(MetricObservation))).scalars().all()
    assert {fact.public_id for fact in facts} == {
        observation["entity_uuid"],
        following["entity_uuid"],
    }
