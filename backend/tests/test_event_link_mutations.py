...
from src.v2.models import ActivityEvent, ActivityMetricLinkV2, EntityRevisionSnapshot, SyncChange
from tests.test_metric_mutations import deletion
from tests.test_sync_v2 import activity_operation, metric_operation, push, register_account, register_device


def event_operation(activity_uuid, **payload):
    return {
        "operation_id": str(uuid4()), "entity_uuid": str(uuid4()),
        "entity_type": "activity_event", "action": "upsert",
        "payload": {
            "activity_uuid": activity_uuid, "event_type": "check_in",
            "occurred_at": "2026-01-01T07:30:00.123456Z", "local_date": "2025-12-31",
            "timezone": "America/Los_Angeles", "metadata": {"note": "offline fact"},
            **payload,
        },
    }
...
def link_operation(activity_uuid, metric_uuid):
    return {
        "operation_id": str(uuid4()), "entity_uuid": str(uuid4()),
        "entity_type": "activity_metric_link", "action": "upsert",
...
@pytest.mark.parametrize("kind,case,code", [
...
async def test_fact_link_rejection_is_cached_without_appending_history(async_session, fact_context, kind, case, code):
    ...
    if case == "delete":
        ...
        candidate["payload"]["activity_uuid" if case == "missing-activity" else "metric_uuid"] = str(uuid4())
    elif case == "source-device":
        candidate["payload"]["source_device_id"] = str(uuid4())
    else:
        candidate["payload"].update(event_type="revert", reverts_event_uuid=str(uuid4()))
    before = {}
    for model in (EntityRevisionSnapshot, SyncChange):
        before[model] = [(row.entity_uuid, row.revision, row.payload_json) for row in
                         (await async_session.execute(select(model).order_by(model.entity_uuid, model.revision))).scalars()]
    ...
    for model, expected in before.items():
        rows = (await async_session.execute(select(model).order_by(model.entity_uuid, model.revision))).scalars()
        assert [(row.entity_uuid, row.revision, row.payload_json) for row in rows] == expected
...
async def test_count_integer_boundaries_and_fact_precision(fact_context, event_type):
    ...
    operations = [event_operation(count["entity_uuid"], event_type=event_type, value=value) for value in values]
    results = await submit(*operations)
    for index, result in enumerate(results):
        if index >= 3:
            assert result["status"] == "rejected" and result["error_code"] == "INVALID_COUNT_VALUE"
            continue
        assert result["status"] == "applied"
        entity = result["entity"]
        assert entity["value"] == int(values[index])
        assert entity["occurred_at"] == "2026-01-01T07:30:00.123456Z"
        assert entity["local_date"] == "2025-12-31" and entity["timezone"] == "America/Los_Angeles"
        assert entity["source_device_id"] == device and entity["metadata"] == {"note": "offline fact"}
...
async def test_link_lifecycle_keeps_flags_revision_tombstone_and_endpoint_identity(async_session, fact_context):
    submit, operations, originals, _, _ = fact_context
    ...
    update["payload"].update(coefficient="-0.123456", show_in_activity_detail=False, prompt_on_complete=True, is_active=False)
    ...
    assert all(changed["entity"][key] == value for key, value in update["payload"].items() if key != "coefficient")
    assert await submit(update) == [{**changed, "status": "already_applied"}]
    other_activity, other_metric = activity_operation(), metric_operation(name="Other metric")
    other_activity["payload"]["title"] = "Other activity"
    assert [r["status"] for r in await submit(other_activity, other_metric)] == ["applied", "applied"]
    for field, endpoint in (("activity_uuid", other_activity), ("metric_uuid", other_metric)):
    ...
    assert stale["error_code"] == "REVISION_CONFLICT" and stale["entity"] == changed["entity"]
    ...
    assert denied["error_code"] == "ENTITY_DELETED" and denied["entity"] == removed["entity"]
    ...
    for model in (EntityRevisionSnapshot, SyncChange):
        rows = (await async_session.execute(select(model).where(model.entity_uuid == link["entity_uuid"]))).scalars()
        assert sorted(row.revision for row in rows) == [1, 2, 3]


async def test_fact_and_link_identical_uuids_remain_account_scoped(test_client, async_session, fact_context):
    submit, operations, originals, _, _ = fact_context
    other = await register_account(test_client, async_session, "other_fact_link_owner")
    device = await register_device(test_client, other["access_token"], "other-fact-link-phone")
...
@pytest.mark.parametrize("mutation", ["event-create", "link-create", "link-update", "link-delete"])
async def test_fact_link_journal_failure_rolls_back_and_preserves_batch_progress(async_session, fact_context, table, mutation):
    submit, operations, originals, _, _ = fact_context
    activity, metric, _, link = operations
    ...
    for model in (EntityRevisionSnapshot, SyncChange):
        rows = (await async_session.execute(select(model).where(model.entity_uuid == failing["entity_uuid"]))).scalars().all()
        assert [row.revision for row in rows] == ([] if mutation.endswith("create") else [1])
    if mutation.endswith("create"):
        model = ActivityEvent if mutation == "event-create" else ActivityMetricLinkV2
        rows = (await async_session.execute(select(model).where(model.public_id == failing["entity_uuid"]))).scalars().all()
        assert rows == []
