...
def activity_operation(
    *,
    ...
    effective_target = target_value if target_value is not None else ("60" if tracking_mode == "duration" else "1")
    return {
        "operation_id": str(operation_id or uuid4()),
        "entity_type": "plan_node",
        "entity_uuid": str(entity_uuid or uuid4()),
...
async def test_stale_structural_edits_on_different_fields_are_three_way_merged(
    ...
    created = await push(test_client, token, device_id, [goal_operation(entity_uuid=goal_uuid)])
...
async def test_stale_activity_decimal_field_merges_instead_of_conflicting(
    ...
    base = activity_operation(entity_uuid=activity_uuid, tracking_mode="count", target_value="1")
    ...
    assert (await push(test_client, token, device_id, [remote])).json()["results"][0]["revision"] == 2
...
async def test_stale_metric_decimal_field_merges_instead_of_conflicting(
    ...
    assert (await push(test_client, token, device_id, [base])).json()["results"][0]["revision"] == 1
...
async def test_decimal_entities_still_conflict_when_the_same_field_diverges(
    ...
    tokens = await register_account(test_client, async_session, "syncmergedecimalconflict")
    token = tokens["access_token"]
    device_id = await register_device(test_client, token, "android-merge-decimal-conflict")
    activity_uuid = uuid4()
...
@pytest.mark.asyncio
async def test_android_configuration_fields_round_trip_without_loss(test_client, async_session):
    ...
    entities = {item["entity_uuid"]: item["payload"] for item in snapshot.json()["changes"]}
...
@pytest.mark.asyncio
async def test_android_incompatible_activity_shapes_are_rejected(test_client, async_session):
    ...
    assert {item["error_code"] for item in response.json()["results"]} == {"INVALID_PAYLOAD"}


@pytest.mark.asyncio
async def test_android_incompatible_icons_and_node_statuses_are_rejected(test_client, async_session):
    ...
    unsupported_icon_activity["payload"].update(title="Unknown icon", icon="future_platform_icon")
    ...
    unsupported_icon_metric = {
        ...
        "action": "upsert",
        "base_revision": None,
        "payload": {"name": "Unsupported icon metric", "unit": "point", "icon": "favorite"},
    }
    ...
    response = await push(
        ...
        token,
        device_id,
        [paused_activity, unsupported_icon_activity, completed_without_result, unsupported_icon_metric],
    )
...
@pytest.mark.asyncio
async def test_android_update_preserves_server_only_plan_fields(test_client, async_session):
    ...
    goal["payload"]["goal"].update({"start_date": "2026-08-01", "due_date": "2026-08-31"})
    ...
    assert [item["status"] for item in created.json()["results"]] == ["applied", "applied"]

    android_goal_update = goal_operation(entity_uuid=goal_uuid, title="Goal edited on Android")
    android_goal_update["base_revision"] = 1
    android_activity_update = activity_operation(parent_uuid=goal_uuid, entity_uuid=activity_uuid)
    ...
    assert [item["status"] for item in updated.json()["results"]] == ["applied", "applied"]
    goal_entity, activity_entity = [item["entity"] for item in updated.json()["results"]]
...
@pytest.mark.asyncio
async def test_single_parent_goal_rules_and_account_isolation(test_client, async_session):
    first = await register_account(test_client, async_session, "firstv2user")
    first_device = await register_device(test_client, first["access_token"], "android-first-001")
    second = await register_account(test_client, async_session, "secondv2user")
    second_device = await register_device(test_client, second["access_token"], "android-second-01")

    first_goal = goal_operation()
    assert (await push(test_client, first["access_token"], first_device, [first_goal])).json()["results"][0]["status"] == "applied"
    ...
    assert (await push(test_client, first["access_token"], first_device, [own_activity])).json()["results"][0]["status"] == "applied"
...
@pytest.mark.asyncio
async def test_goal_delete_detaches_children_and_propagates_tombstone(test_client, async_session):
    ...
    operations = [(c["entity_uuid"], c["operation"], c["revision"]) for c in pull.json()["changes"]]
    assert (activity["entity_uuid"], "upsert", 2) in operations
    assert (goal["entity_uuid"], "delete", 2) in operations


@pytest.mark.asyncio
async def test_activity_event_validation_revert_and_bootstrap(test_client, async_session):
    ...
    event_changes = [
        change for change in bootstrap.json()["changes"] if change["entity_type"] == "activity_event"
    ]
    assert len(event_changes) == 2

    original = next(change for change in event_changes if change["entity_uuid"] == str(event_uuid))
    occurred_at = original["payload"]["occurred_at"]
    assert occurred_at == "2026-08-03T00:30:00Z"
    assert datetime.fromisoformat(occurred_at.replace("Z", "+00:00")).utcoffset() is not None
...
@pytest.mark.asyncio
async def test_external_count_event_round_trip_is_utc_and_unique(test_client, async_session):
    ...
    count_event = next(
        change for change in bootstrap.json()["changes"]
...
@pytest.mark.asyncio
async def test_operation_id_cannot_be_reused_with_different_body(test_client, async_session):
    ...
    assert (await push(test_client, token, device_id, [first])).json()["results"][0]["status"] == "applied"
...
@pytest.mark.asyncio
async def test_bootstrap_excludes_facts_and_links_whose_parent_is_deleted(test_client, async_session):
    ...
    created = await push(test_client, token, device_id, [activity, metric, observation, link])
...
@pytest.mark.asyncio
async def test_deleted_activity_metric_link_can_be_created_again(test_client, async_session):
    ...
    assert (await push(test_client, token, device_id, [unlink])).json()["results"][0]["status"] == "applied"
...
@pytest.mark.asyncio
async def test_conflict_always_contains_authoritative_revision_and_entity(test_client, async_session):
