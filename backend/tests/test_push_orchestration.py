...
from src.v2.models import ClientDevice, EntityRevisionSnapshot, PlanNode, SyncChange, SyncCursor, SyncOperation
...
from tests.test_sync_v2 import activity_operation, goal_operation, metric_operation, push, register_account, register_device
...
async def push_context(test_client, async_session):
    ...
    user = (await async_session.execute(select(User).where(User.username == "push_audit_owner"))).scalar_one()
...
def hide_initial_operation_lookup(monkeypatch, session, operation_id):
    ...
    async def wrapped(statement, *args, **kwargs):
        is_operation = any(d.get("entity") is SyncOperation for d in getattr(statement, "column_descriptions", []))
        if not hidden and is_operation and operation_id in statement.compile().params.values():
            hidden.append(True)
...
async def test_unique_insert_race_reuses_result_without_mutating_it(test_client, async_session, monkeypatch, push_context, status, different_hash):
    ...
    await async_session.commit()
    row = (await async_session.execute(select(SyncOperation).where(SyncOperation.operation_id == operation["operation_id"]))).scalar_one()
    stored = row.result_json
    candidate = deepcopy(operation)
    if different_hash:
        candidate["payload"]["description"] = "Different request with the same ID"
    hidden = hide_initial_operation_lookup(monkeypatch, async_session, operation["operation_id"])
    ...
    assert hidden == [True]
    if different_hash:
        assert result["status"] == "rejected" and result["error_code"] == "OPERATION_ID_REUSED"
    else:
        assert result == ({**first, "status": "already_applied"} if status == "applied" else first)
...
async def test_incomplete_replay_aborts_entire_batch_and_allows_safe_retry(async_session, async_engine, test_client, monkeypatch, push_context, raced, incomplete):
    owner, token, writer, _ = push_context
    operation = goal_operation(title="Pending operation")
    device = (await async_session.execute(select(ClientDevice).where(ClientDevice.public_id == writer))).scalar_one()
    pending = SyncOperation(user_id=owner, device_id=device.id, operation_id=operation["operation_id"],
                            request_hash=operation_hash(SyncOperationRequest.model_validate(operation)),
                            entity_type="plan_node", entity_uuid=operation["entity_uuid"], action="upsert",
                            status="processing" if incomplete == "processing" else "applied", result_json="{}")
    ...
    with pytest.raises(DomainError) as failure:
        async with sessions.begin() as session:
            if raced:
                hidden = hide_initial_operation_lookup(monkeypatch, session, operation["operation_id"])
            user = await session.get(User, owner)
            await process_push(user, SyncPushRequest.model_validate({"device_id": writer, "operations": [before, operation, after]}), session)
...
async def test_operation_insert_constraint_isolated_from_following_item(test_client, async_session, push_context):
    _, token, writer, _ = push_context
    blocked, later = goal_operation(title="Blocked operation record"), goal_operation(title="After operation failure")
    ...
    assert rejected["error_code"] == "CONSTRAINT_VIOLATION" and rejected["status"] == "rejected"
...
async def test_final_persistence_failure_rolls_back_business_and_replay_state(async_session, async_engine, test_client, push_context, failure_point):
    owner, token, writer, _ = push_context
    operation = goal_operation(title="Retry after final persistence failure")
    target = "UPDATE ON sync_operations" if failure_point == "result" else "INSERT ON sync_cursors"
    await async_session.execute(text(
    ...
    with pytest.raises(IntegrityError, match="injected final persistence failure"):
        async with sessions.begin() as session:
            user = await session.get(User, owner)
            await process_push(user, SyncPushRequest.model_validate({"device_id": writer, "operations": [operation]}), session)
    async with sessions.begin() as session:
        for model in (PlanNode, EntityRevisionSnapshot, SyncChange, SyncOperation, SyncCursor):
            assert (await session.execute(select(model))).scalars().all() == []
...
async def test_secondary_can_only_finalize_task_with_an_effective_check_in(test_client, async_session, push_context, reverted):
    _, token, writer, secondary = push_context
    task = activity_with_rule({"type": "once"})
    assert (await push(test_client, token, writer, [task])).json()["results"][0]["status"] == "applied"
    event = event_operation(task["entity_uuid"])
    operations = [event]
    if reverted:
        operations.append(event_operation(task["entity_uuid"], event_type="revert", reverts_event_uuid=event["entity_uuid"]))
    ...
    if reverted:
        assert results[-1]["status"] == "rejected" and results[-1]["error_code"] == "DEVICE_CAPABILITY_DENIED"
    else:
    ...
    assert retry.json()["results"] == [{**r, "status": "already_applied"} if r["status"] == "applied" else r for r in results]
    if reverted:
        ...
        checked = await push(test_client, token, secondary, [event_operation(task["entity_uuid"]), remove, deletion(task)])
        checked_results = checked.json()["results"]
        assert [r["status"] for r in checked_results] == ["applied", "rejected", "applied"]
        assert checked_results[1] == results[-1]
        assert checked_results[2]["revision"] == 2


@pytest.mark.parametrize("kind", ["unfinished", "recurring", "goal", "metric", "missing", "rename"])
async def test_secondary_finalization_exception_does_not_grant_structure_access(test_client, async_session, push_context, kind):
    _, token, writer, secondary = push_context
    original = metric_operation() if kind == "metric" else goal_operation() if kind == "goal" else activity_operation() if kind == "recurring" else activity_with_rule({"type": "once"})
    assert (await push(test_client, token, writer, [original])).json()["results"][0]["status"] == "applied"
    if kind in {"recurring", "rename"}:
        assert (await push(test_client, token, secondary, [event_operation(original["entity_uuid"])])).json()["results"][0]["status"] == "applied"
    ...
    assert denied["status"] == "rejected" and denied["error_code"] == "DEVICE_CAPABILITY_DENIED"
