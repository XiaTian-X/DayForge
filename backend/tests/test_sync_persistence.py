...
async def snapshot_setup(test_client, async_session):
    ...
    user = (await async_session.execute(select(User).where(User.username == "snapshot_owner"))).scalar_one()
    await async_session.commit()
    return user.id, token, device, batch, results


@pytest.mark.parametrize("index", range(7), ids=["goal", "check", "duration", "metric", "event", "observation", "link"])
async def test_authoritative_snapshot_is_scoped_by_owner_even_with_identical_public_ids(
    test_client, async_session, snapshot_setup, index,
):
    owner, _, _, batch, results = snapshot_setup
    operation = SyncOperationRequest.model_validate(batch["operations"][index])
    other_account = await register_account(test_client, async_session, "other_snapshot_owner")
    other = (await async_session.execute(select(User).where(User.username == "other_snapshot_owner"))).scalar_one()
    assert await current_entity_snapshot(async_session, other.id, operation) == (None, None)

    other_device = await register_device(test_client, other_account["access_token"], "other-snapshot-phone")
    ...
    response = await push(test_client, other_account["access_token"], other_device, other_batch["operations"])
    assert response.status_code == 200, response.text
    other_results = response.json()["results"]
    assert [result["status"] for result in other_results] == ["applied"] * 7
    await async_session.commit()

    for user_id, expected in ((owner, results[index]), (other.id, other_results[index])):
        revision, payload = await current_entity_snapshot(async_session, user_id, operation)
...
@pytest.mark.parametrize("index", [0, 1, 3, 5, 6], ids=["goal", "activity", "metric", "observation", "link"])
async def test_authoritative_snapshot_keeps_tombstones_for_conflict_resolution(
    ...
    deletion = {
        "operation_id": str(uuid4()), "entity_type": original["entity_type"],
        "entity_uuid": original["entity_uuid"], "action": "delete", "base_revision": 1,
    ...
    revision, payload = await current_entity_snapshot(
        async_session, owner, SyncOperationRequest.model_validate(deletion),
    )
...
async def test_timer_snapshot_preserves_sorted_day_allocations_and_journal_payload(
    ...
    response = await test_client.post(
        "/api/v2/timers/commands", headers={"Authorization": f"Bearer {token}"}, json=commands,
    )
    assert response.status_code == 200, response.text
    assert [result["status"] for result in response.json()["results"]] == ["applied"] * 4
    await async_session.commit()
    operation = SyncOperationRequest(
        operation_id=uuid4(), entity_type="activity_event", action="upsert",
    ...
    assert payload["day_allocations"] == [
        {"local_date": "2026-08-03", "timezone": "Asia/Shanghai", "duration_milliseconds": 30000},
        {"local_date": "2026-08-04", "timezone": "Asia/Shanghai", "duration_milliseconds": 30000},
    ]
    change = (await async_session.execute(select(SyncChange).where(
        SyncChange.recipient_user_id == owner, SyncChange.entity_uuid == str(operation.entity_uuid),
    ))).scalar_one()
    snapshot = (await async_session.execute(select(EntityRevisionSnapshot).where(
    ...
    assert snapshot.payload_hash == hashlib.sha256(change.payload_json.encode("utf-8")).hexdigest()


async def test_revert_snapshot_keeps_its_original_event_reference(test_client, async_session, snapshot_setup):
    owner, token, device, batch, _ = snapshot_setup
    event = batch["operations"][4]
    revert = {
        "operation_id": str(uuid4()), "entity_uuid": str(uuid4()), "entity_type": "activity_event",
        "action": "upsert", "payload": {
            "activity_uuid": event["payload"]["activity_uuid"], "event_type": "revert",
            "reverts_event_uuid": event["entity_uuid"], "occurred_at": "2026-11-01T06:30:00Z",
            "local_date": "2026-11-01", "timezone": "America/New_York", "note": "Reverted fact",
        },
    }
    ...
    await async_session.commit()
    revision, payload = await current_entity_snapshot(
        async_session, owner, SyncOperationRequest.model_validate(revert),
    )
    assert revision == 1
    assert payload == result["entity"]
...
async def test_journal_constraint_failure_rolls_back_entity_and_allows_next_batch_item(
    test_client, async_session, table, mode,
):
    account = await register_account(test_client, async_session, "journal_failure_owner")
    token = account["access_token"]
    ...
    prior = None
    if mode != "create":
        prior = goal_operation(entity_uuid=failing["entity_uuid"], title="Preserved original")
    ...
    expected_operations = {failing["operation_id"]: "rejected", succeeding["operation_id"]: "applied"}
    if prior:
        expected_nodes[prior["entity_uuid"]] = ("Preserved original", 1, None)
        expected_operations[prior["operation_id"]] = "applied"
    assert {node.public_id: (node.title, node.revision, node.deleted_at) for node in nodes} == expected_nodes
    assert (await async_session.execute(select(func.count()).select_from(GoalDetail))).scalar_one() == len(expected_nodes)
    for model in (EntityRevisionSnapshot, SyncChange):
        rows = (await async_session.execute(select(model))).scalars().all()
        assert {row.entity_uuid: row.revision for row in rows} == {key: 1 for key in expected_nodes}
    ...
    assert replay.json()["results"] == [rejected, {**applied, "status": "already_applied"}]
    for model in (EntityRevisionSnapshot, SyncChange):
        assert (await async_session.execute(select(func.count()).select_from(model))).scalar_one() == len(expected_nodes)


async def test_outer_rollback_removes_entity_history_cursor_and_idempotency_result(
    test_client, async_session, async_engine,
):
    account = await register_account(test_client, async_session, "outer_rollback_owner")
    device = await register_device(test_client, account["access_token"], "outer-rollback-phone")
    user = (await async_session.execute(select(User).where(User.username == "outer_rollback_owner"))).scalar_one()
    user_id = user.id
    await async_session.commit()
    operation = goal_operation()
    request = SyncPushRequest.model_validate({"device_id": device, "operations": [operation]})
    ...
    with pytest.raises(AbortRequest):
        async with sessions.begin() as transaction:
            ...
            for model in (PlanNode, GoalDetail, EntityRevisionSnapshot, SyncChange, SyncOperation, SyncCursor):
                assert (await transaction.execute(select(func.count()).select_from(model))).scalar_one() == 1
            raise AbortRequest()

    async with sessions() as reader:
        for model in (PlanNode, GoalDetail, EntityRevisionSnapshot, SyncChange, SyncOperation, SyncCursor):
            assert (await reader.execute(select(func.count()).select_from(model))).scalar_one() == 0
