...
async def hierarchy(test_client, async_session):
    ...
    for mode, countdown in (
        ...
        child = activity_operation(
            parent_uuid=goal["entity_uuid"], tracking_mode=mode, is_countdown=countdown
        )
    ...
    nodes = (
        (
            await session.execute(
                select(PlanNode).execution_options(populate_existing=True)
            )
        )
        .scalars()
        .all()
...
async def test_goal_deletion_updates_each_tracking_variant_once_and_is_account_scoped(
    test_client, async_session, hierarchy, policy
):
    ...
    other = await register_account(
        test_client, async_session, "foreign_hierarchy_owner"
    )
    other_device = await register_device(
        test_client, other["access_token"], "foreign-hierarchy-phone"
    )
    foreign = await push(
        test_client, other["access_token"], other_device, [goal, children[0]]
    )
    assert [item["status"] for item in foreign.json()["results"]] == [
        "applied",
        "applied",
    ]
    foreign_before = await test_client.get(
    ...
    assert {item["entity_uuid"] for item in changed[:-1]} == {
        child["entity_uuid"] for child in children
    }
    ...
    for item in changed[:-1]:
        ...
        assert item["operation"] == (
            "delete" if policy == "cascade_children" else "upsert"
        )
    boot = await test_client.get(
        "/api/v2/sync/bootstrap",
        headers={"Authorization": f"Bearer {token}"},
    ...
    foreign_after = await test_client.get(
        "/api/v2/sync/bootstrap",
        headers={"Authorization": f"Bearer {other['access_token']}"},
        params={"device_id": other_device},
    )
...
async def test_goal_with_children_requires_an_explicit_valid_deletion_policy(
    async_session, hierarchy, policy
):
    ...
    assert (
        ...
        and rejected["error_code"] == "CHILD_POLICY_REQUIRED"
    )
    assert await submit(request) == [rejected]
    assert await current_nodes(async_session) == originals
    assert len(
        (await async_session.execute(select(SyncChange))).scalars().all()
    ) == len(originals)


@pytest.mark.parametrize(
    "index", range(5), ids=["check", "count-up", "count-down", "timer-up", "timer-down"]
)
async def test_reparent_and_detach_preserve_tracking_details_and_survive_old_parent_deletion(
    async_session, hierarchy, index
):
    ...
    assert (await current_nodes(async_session))[child["entity_uuid"]] == detached[
        "entity"
    ]
...
async def test_invalid_hierarchy_updates_leave_nodes_and_details_unchanged(
    async_session, hierarchy, case
):
    submit, goal, other_goal, children, originals, _, _ = hierarchy
    ...
    if case == "missing":
        ...
        request["payload"]["parent_uuid"] = children[1 if case == "activity" else 0][
            "entity_uuid"
        ]
        code = "INVALID_PARENT"
...
async def test_goal_delete_journal_failure_rolls_back_every_child_and_parent(
    async_session, hierarchy, table, policy, failure_at
):
    submit, goal, _, children, originals, _, _ = hierarchy
    target = (
        goal
        if failure_at == "parent"
        else children[0 if failure_at == "first-child" else -1]
    )
    ...
    for model in (SyncChange, EntityRevisionSnapshot):
        ...
        assert {
            row.entity_uuid: json.loads(row.payload_json) for row in rows
        } == expected
        assert all(row.revision == 1 for row in rows)
    assert await submit(request, following) == [
        rejected,
