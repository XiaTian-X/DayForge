...
async def journal(session):
    ...
    for model in (EntityRevisionSnapshot, SyncChange):
        rows = (await session.execute(select(model).order_by(model.entity_uuid, model.revision))).scalars().all()
        result[model.__name__] = [(row.entity_uuid, row.revision, row.operation, row.payload_json) for row in rows]
    return result


@pytest.mark.parametrize("payload", INVALID_PAYLOADS)
async def test_invalid_policy_is_cached_without_mutation_and_other_batch_items_succeed(async_session, hierarchy, payload):
    ...
    expected = {**originals, earlier["entity_uuid"]: first["entity"], later["entity_uuid"]: last["entity"]}
    ...
    assert await submit(earlier, request, later) == [
        {**first, "status": "already_applied"}, rejected, {**last, "status": "already_applied"},
    ]
    ...
    assert reused["status"] == "rejected" and reused["error_code"] == "OPERATION_ID_REUSED"
    assert await current_nodes(async_session) == expected
    ...
    assert await submit(request, corrected) == [rejected, {**applied, "status": "already_applied"}]
    assert await journal(async_session) == corrected_history


@pytest.mark.parametrize("policy", [[], {}], ids=["array", "object"])
@pytest.mark.parametrize("target_kind", ["empty-goal", "only-deleted-children", "activity"])
async def test_unused_policy_does_not_change_deletion_without_active_children(async_session, hierarchy, policy, target_kind):
    submit, goal, empty_goal, children, _, _, _ = hierarchy
    if target_kind == "only-deleted-children":
        assert all(item["status"] == "applied" for item in await submit(*(delete_node(child) for child in children)))
        target = goal
    else:
...
async def test_existing_entity_and_revision_checks_precede_policy_validation(async_session, hierarchy, policy, state):
    ...
    result, = await submit(request)
    if state == "missing":
        assert result["status"] == "rejected" and result["error_code"] == "ENTITY_NOT_FOUND"
    elif state == "stale":
        assert result["status"] == "conflict" and result["error_code"] == "REVISION_CONFLICT"
        assert result["revision"] == 1 and result["entity"] == before[goal["entity_uuid"]]
    else:
        assert result["status"] == "applied" and result["revision"] == 2
