...
from src.v2.models import ClientDevice, SyncCursor, SyncOperation, TimerCommand, UserSyncPolicy
...
async def device_context(test_client, async_session):
    ...
    payload = {
        "installation_id": "device-boundary-install", "protocol_version": 4,
        "platform": "desktop", "device_class": "interactive",
        "app_version": "1.0", "display_name": "Original device",
    }
    response = await test_client.post("/api/v2/devices/register", headers=headers, json=payload)
...
@pytest.mark.parametrize("metadata", [{"app_version": "2.0", "display_name": "Renamed device"}, {}])
async def test_reregistration_only_refreshes_metadata_and_seen_time(test_client, async_session, device_context, metadata):
    _, headers, payload, device, original = device_context
    request = {key: value for key, value in payload.items() if key not in {"app_version", "display_name"}}
    request.update(metadata)
    response = await test_client.post("/api/v2/devices/register", headers=headers, json=request)
    assert response.status_code == 200, response.text
    current = response.json()
    for field in ("device_id", "installation_id", "platform", "device_class", "is_primary_editor",
                  "structural_edit_enabled", "capability_revision", "capabilities"):
...
async def test_rejected_registration_preserves_existing_device_and_policy(test_client, async_session, device_context, case, code, status):
    _, headers, payload, device, _ = device_context
    request = {**payload, "app_version": "Must not persist", "display_name": "Rejected rename"}
    if case == "platform":
...
async def test_same_installation_id_belongs_to_separate_accounts(test_client, async_session, device_context):
    _, _, payload, _, original = device_context
    other = await account(test_client, async_session, "other_device_boundary_owner")
    response = await test_client.post(
        "/api/v2/devices/register", headers={"Authorization": f"Bearer {other['access_token']}"}, json=payload,
    )
    assert response.status_code == 200, response.text
    ...
    assert {row["user_id"] for row in state[ClientDevice]} == {row["user_id"] for row in state[UserSyncPolicy]}
    assert {row["primary_editor_device_id"] for row in state[UserSyncPolicy]} == {row["id"] for row in state[ClientDevice]}


@pytest.mark.parametrize("kind", ["missing", "foreign", "revoked"])
async def test_all_sync_and_timer_entries_reject_unavailable_devices_without_writes(test_client, async_session, device_context, kind):
    ...
    cases = [
        ...
        ("GET", "/api/v2/sync/changes", {"params": {"device_id": device_id, "cursor": 0}}),
        ...
        ("POST", f"/api/v2/timers/{timer_id}/heartbeat", {"json": {"device_id": device_id, "control_generation": 1}}),
    ]
...
async def test_device_registration_and_primary_assignment_rollback_with_caller(async_session, test_client):
    owner = await account(test_client, async_session, "device_rollback_owner")
    request = DeviceRegisterRequest(installation_id="rollback-registration", platform="android", protocol_version=4)
    ...
    assert await device_snapshot(async_session) == {ClientDevice: [], UserSyncPolicy: []}
...
async def test_active_device_lookup_seen_time_remains_caller_transactional(async_session, device_context):
