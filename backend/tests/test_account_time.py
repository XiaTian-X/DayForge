...
def test_normalization_preserves_dst_fold_and_microseconds(fold, expected_hour):
    value = datetime(2026, 11, 1, 1, 30, 0, 123456, tzinfo=ZoneInfo("America/Los_Angeles"), fold=fold)
...
def test_admin_schema_normalizes_instants_without_mutating_model(offset):
    ...
    user = User(id=1, username="probe", password_hash="private", created_at=value, updated_at=value)
    ...
    for field in ("created_at", "updated_at"):
        assert response.model_dump(mode="json")[field] == expected.isoformat().replace("+00:00", "Z")
    assert user.created_at == value
    assert user.created_at.tzinfo == value.tzinfo
    assert "password_hash" not in response.model_dump()


async def test_sqlite_round_trip_and_onupdate_preserve_utc(async_session, server_timezone):
    ...
    raw = (await async_session.execute(text("SELECT created_at FROM users WHERE id=:id"), {"id": user.id})).scalar_one()
    assert datetime.fromisoformat(raw) == created_at.replace(tzinfo=None)

    token = ApiToken(user_id=user.id, name="utc", prefix="probe", token_hash="synthetic",
                     expires_at=utc_now() + timedelta(days=1), last_used_at=utc_now())
    original = {field: getattr(token, field) for field in ("created_at", "expires_at", "last_used_at")}
    ...
    for field, value in original.items():
        ...
        assert response.model_dump(mode="json")[field] == value.isoformat().replace("+00:00", "Z")


@pytest.mark.parametrize("admin_issues_token", [False, True])
async def test_account_and_token_http_round_trip(test_client, async_session, server_timezone, admin_issues_token, monkeypatch):
    admin = User(username="utc-admin", password_hash=get_password_hash("test-password"), is_admin=True)
    async_session.add(admin)
    await async_session.commit()
    login = await test_client.post("/api/v1/auth/login", json={"username": "utc-admin", "password": "test-password"})
    assert login.status_code == 200
    admin_headers = {"Authorization": f"Bearer {login.json()['access_token']}"}
    before = utc_now()
    created = await test_client.post("/api/v1/admin/users", headers=admin_headers,
    ...
    login = await test_client.post("/api/v1/auth/login", json={"username": "utc_member", "password": "test-password"})
    assert login.status_code == 200
    member_headers = {"Authorization": f"Bearer {login.json()['access_token']}"}
    admin_path = f"/api/v1/admin/users/{user['id']}/tokens"
    create_path, headers = (admin_path, admin_headers) if admin_issues_token else ("/api/v1/auth/tokens", member_headers)
    before = utc_now()
    created = await test_client.post(create_path, headers=headers, json={"name": "clock", "expires_in_days": 7})
    assert created.status_code == 201
    token = created.json()
    ...
    assert before + timedelta(days=7) <= datetime.fromisoformat(token["expires_at"]) <= utc_now() + timedelta(days=7)
    for path, headers in (("/api/v1/auth/tokens", member_headers), (admin_path, admin_headers)):
        listed = await test_client.get(path, headers=headers)
        assert listed.status_code == 200
        assert listed.json() == [{key: value for key, value in token.items() if key != "token"}]

    before = utc_now()
    used = await test_client.get("/api/v1/auth/tokens", headers={"Authorization": f"Token {token['token']}"})
    ...
    with monkeypatch.context() as clock:
        clock.setattr("src.tokens.service.utc_now", lambda: datetime.fromisoformat(token["expires_at"]))
        expired = await test_client.get("/api/v1/auth/tokens", headers={"Authorization": f"Token {token['token']}"})
        assert expired.status_code == 401

    changed = await test_client.put(f"/api/v1/admin/users/{user['id']}/status", headers=admin_headers,
    ...
    assert listed.status_code == 200
    assert next(item for item in listed.json() if item["id"] == user["id"]) == changed.json()


async def test_legacy_naive_rows_are_rendered_without_rewrite(test_client, async_session, server_timezone):
    old = datetime(2020, 1, 1, 23, 59, 59, 123456)
    user = User(username="legacy-utc", password_hash=get_password_hash("test-password"), is_admin=True,
                created_at=old, updated_at=old)
    async_session.add(user)
    await async_session.flush()
    token = ApiToken(user_id=user.id, name="legacy", prefix="probe", token_hash="synthetic-legacy",
                     created_at=old, last_used_at=old)
    async_session.add(token)
    await async_session.commit()
    raw_query = text("SELECT created_at, last_used_at, expires_at FROM api_tokens WHERE id=:id")
    before = (await async_session.execute(raw_query, {"id": token.id})).one()
    login = await test_client.post("/api/v1/auth/login", json={"username": "legacy-utc", "password": "test-password"})
    assert login.status_code == 200
