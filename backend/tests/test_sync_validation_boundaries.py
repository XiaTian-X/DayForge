...
@pytest.mark.parametrize(
    "index", [0, 1, 3, 6], ids=["goal", "activity", "metric", "link"]
)
@pytest.mark.parametrize("base_state", ["current", "stale", "unavailable"])
@pytest.mark.parametrize("shape", ["empty", "invalid-field"])
async def test_invalid_structural_payload_is_cached_and_does_not_abort_batch(
    test_client, async_session, snapshot_setup, index, base_state, shape
):
    owner, token, device, batch, _ = snapshot_setup
    original = batch["operations"][index]
    remote = edit(original)
    remote["payload"].update(
        {"coefficient": "2"} if index == 6 else {"description": "Remote edit"}
    )
    assert (await push(test_client, token, device, [remote])).json()["results"][0][
        "revision"
    ] == 2
    ...
    before, after = (
        goal_operation(title="Valid before"),
        goal_operation(title="Valid after"),
    )
    ...
    assert (await push(test_client, token, device, [invalid])).json()["results"][
        0
    ] == results[1]


@pytest.mark.parametrize(
    "field,value", [("start_date", "2026-09-20"), ("due_date", "2026-08-20")]
)
@pytest.mark.parametrize("stale", [False, True])
async def test_partial_goal_dates_validate_final_retained_range(
    merge_client, async_session, field, value, stale
):
    ...
    assert (
        rejected["status"] == "rejected" and rejected["error_code"] == "INVALID_PAYLOAD"
    )
    ...
    assert (
        str(detail.start_date) == "2026-09-01" and str(detail.due_date) == "2026-09-10"
    )
    ...
    assert (
        ...
        and applied["revision"] == baseline["revision"] + 1
    )
    assert await merge_client(invalid) == rejected
...
@pytest.mark.parametrize("stale", [False, True])
async def test_valid_partial_goal_dates_preserve_omission_and_explicit_null(
    merge_client, async_session, field, value, stale
):
    original = goal_operation()
    original["payload"]["goal"].update(start_date="2026-09-01", due_date="2026-09-10")
